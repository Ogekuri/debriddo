...
- **REQ-571**: La pipeline di ricerca deve costruire le stringhe di ricerca per film usando titolo, anno e tag lingua iterando le lingue richieste da `self.__config['languages']` (non da `movie.languages`), con fallback controllato da `SEARCHE_FALL_BACK`.  
  ID originale: `N/A`.
  Comportamento atteso: la ricerca primaria deve essere sempre eseguita anche quando `SEARCHE_FALL_BACK` e' false. Se `self.__config['languages']` e' vuoto o mancante, deve essere eseguita una sola ricerca primaria senza `lang_tag` (`<title> <year>`). Se `self.__config['languages']` contiene lingue, per ogni lingua richiesta la query primaria e' `<title> <year> <lang_tag>` salvo il caso in cui la lingua dell'indexer sia diversa da `en` e uguale alla lingua richiesta, dove la query deve essere `<title> <year>` (senza tag). In tutti gli altri casi il tag deve essere ottenuto da `self.__language_tags[lang]` (se presente). La selezione del titolo localizzato deve essere guidata dalla lingua del ciclo estratta da `self.__config['languages']` e non deve usare `movie.languages` come sorgente di controllo per il ciclo o per la mappatura lingua->titolo. Se `SEARCHE_FALL_BACK` e' true e nessuna ricerca primaria produce risultati, deve essere eseguita una ricerca fallback con `<title> <lang_tag>` oppure `<title>` con le stesse regole di composizione del tag.  
  Criteri di accettazione: `__search_movie_indexer()` legge le lingue da `self.__config['languages']`, non usa `movie.languages` per il ciclo lingue e la mappatura lingua->titolo, costruisce `lang_tag` vuoto quando non ci sono lingue richieste o quando `indexer.language != 'en' and indexer.language == lang`, esegue sempre le query primarie `<title> <year>[ <lang_tag>]`, esegue le query fallback `<title>[ <lang_tag>]` solo se `SEARCHE_FALL_BACK` e' true e l'insieme risultati primari e' vuoto, e concatena i risultati in ordine di esecuzione.  
...
- **REQ-572**: La pipeline di ricerca deve eseguire una ricerca primaria multipla per serie TV concatenando i risultati di episodio specifico, pack multi-episodi e stagione specifica, iterando le lingue richieste da `self.__config['languages']` (non da `series.languages`), con fallback controllato da `SEARCHE_FALL_BACK`.  
  ID originale: `N/A`.
  Comportamento atteso: la ricerca primaria deve essere sempre eseguita anche quando `SEARCHE_FALL_BACK` e' false. Se `self.__config['languages']` e' vuoto o mancante, deve essere eseguito un solo ciclo primario senza `lang_tag` con due query concatenate: a) `<title> <season><episode>`, b) `<title> <season>E01-`. Se `self.__config['languages']` contiene lingue, per ogni lingua richiesta devono essere eseguite e concatenate tre query primarie: a) `<title> <season><episode> <lang_tag>`, b) `<title> <season>E01- <lang_tag>`, c) `<title> "<SeasonLocalized>" <season_number> <lang_tag>`, dove `<SeasonLocalized>` usa la localizzazione della lingua corrente quando presente (es. `Stagione` per `it`) altrimenti `Season`, e `<season_number>` e' il numero intero stagione. La selezione del titolo localizzato deve essere guidata dalla lingua del ciclo estratta da `self.__config['languages']` e non deve usare `series.languages` come sorgente di controllo per il ciclo o per la mappatura lingua->titolo. Per tutte le query primarie, il tag deve essere omesso quando `indexer.language != 'en' and indexer.language == lang`; negli altri casi il tag deve essere ottenuto da `self.__language_tags[lang]` (se presente). Se `SEARCHE_FALL_BACK` e' true e nessuna query primaria produce risultati, deve essere eseguita una ricerca fallback con `<title> <lang_tag>` oppure `<title>` con le stesse regole di composizione del tag.  
  Criteri di accettazione: `__search_series_indexer()` legge le lingue da `self.__config['languages']`, non usa `series.languages` per il ciclo lingue e la mappatura lingua->titolo, genera sempre le query primarie episodio e pack (`<season>E01-`), genera la query primaria stagione localizzata solo quando esistono lingue in `self.__config['languages']`, costruisce `lang_tag` vuoto quando non ci sono lingue richieste o quando `indexer.language != 'en' and indexer.language == lang`, concatena i risultati delle query primarie in ordine, ed esegue fallback `<title>[ <lang_tag>]` solo se `SEARCHE_FALL_BACK` e' true e il totale risultati primari e' zero.  
